...
- `src/cgppl/lexer.py`: strict lexer with source spans, comments, literals, keywords, operators, and graph-construction keywords.
- `src/cgppl/ast.py`: typed AST nodes for the first implemented subset, including sequential statement blocks and graph construction statements.
- `src/cgppl/parser.py`: recursive-descent parser for `program Name { ... }`, rule declarations, `skip`, `fail`, rule calls, graph requirement statements, graph delete statements, graph add statements, and brace-delimited statement blocks.
...
- `src/cgppl/runtime.py`: minimal runtime for `skip`, `fail`, rule-call dispatch, `require node` / `require edge` graph inspection, `delete node` / `delete edge` graph mutation, `add node` / `add edge` graph construction, and sequential block execution.
- `src/cgppl/cli.py`: `cgppl lex`, `cgppl parse`, `cgppl validate`, and `cgppl run` commands.
- `tests/`: pytest coverage for lexer, parser, semantic validation, graph IR behavior, runtime dispatch, graph inspection, graph mutation, graph construction, sequential execution, and CLI graph execution.
...
Use the graph/runtime API from Python:
    ...
    program = parse_program('program Demo { rule main => { require node "n1"; delete node "n1"; add node "n3"; } }')
    graph = Graph.empty().add_node(Node("n1"))
    result = execute_program(program, graph)
    assert result.graph.node_ids == ("n3",)
...
Add graph attribute mutation so constructed nodes and edges can carry useful data:

1. extend the AST and parser with `set node "id" attr "name" = "value";` and `set edge "id" attr "name" = "value";`,
2. parse string, integer, and boolean literal values into typed AST literals,
3. execute those statements through `Node.with_attr` / `Edge.with_attr` plus `Graph.replace_node` / `Graph.replace_edge`,
4. add CLI tests for a block that creates a node and assigns an attribute before emitting result JSON.
